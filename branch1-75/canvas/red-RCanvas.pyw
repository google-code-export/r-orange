...
class OrangeCanvasDlg(QMainWindow):
    ...
    def menuItemReport(self):
        ...
        print fileDir2
        ## we need to set up some things first to make the document look nice.  We are planning to use a three box approach to this.  The screen will be divided unto three parts, top-left, bottom-left, and right.  Top-left will be a picture of the schema, bottom-left will be a contents menu, and right will be the output.  
        dataDir = os.path.splitext("Data-"+str(os.path.split(name)[1]))[0]
        mainPage = r'<html><head></head><body><frameset cols="30%, 70%"><frame src = "'+str(dataDir)+ '/rightPannel.html'+'" /><frame src = "'+str(dataDir)+'/contentPannel.html'+'" name = "mainPage"/></frameset></body></html>'

        file = open(name, "wt")
        file.write(mainPage)
        file.close()
        
        leftPage = r'<html><head></head><body><frameset rows="30%, 70%"><frame src = "'+str(dataDir)+ '/canvas-image.html'+'" /><frame src = "'+str(dataDir)+'/contents.html'+'" /></frameset>leftPage</body></html>'
        
        file = open(os.path.join(fileDir2, 'rightPannel.html'), 'wt')
        file.write(leftPage)
        file.close()
        
        canvasImagePage = r'<html><head></head><body><img src="canvas-image.png" alt="Schema Image" height = "350" width = "500"/></body></html>'
        file = open(str(os.path.join(fileDir2, 'canvas-image.html')).replace('\\','/'), 'wt')
        file.write(canvasImagePage)
        file.close()
        ...
        text = '<html><body><h2>Red-R Report compiled on '+str(datetime.date.today())+'</h2>'
        text += '<h3>Schema Information</h3>'
        text += 'Red-R reports are generated by Red-R for reporting.  In general widgets (Red-R modules) should have their own reporting formats.  When these do not exist Red-R will generate a report for that widget.  </br> When sharing reports it is important to remember to send the "Data" file with the report.  The information is relative to the location of the report file so be sure to keep the two in the same folder.  </br> More information on Red-R can be found at <a href="www.red-r.org">Red-R.org</a>.</br>  The report begins here.'
        text += self.schema.getReportText(fileDir2)
        text += '</body></html>'
        
        file = open(str(os.path.join(fileDir2, 'contentPannel.html')).replace('\\','/'), "wt")
        file.write(text)
        file.close()
