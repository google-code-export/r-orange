...
class colMatcher(OWRpy): 
    ...
    def commitFunction(self):
        ...
        if unicode(self.glSelector.getChecked()) == 'Greater Than':
            gl = 0
        else:
            gl = 1
        self.R(self.Rvariables['compColumn']+'<-NULL', wantType = 'NoConversion')
        names = [unicode(i.text().toAscii()) for i in self.columnBox.selectedItems()]
        compMatrix = self.R('as.matrix('+self.RFunctionParam_x+'[,c("'+'","'.join(names)+'")])')
        for l in compMatrix:
            for i in range(len(l)):
                if gl:
                    if l[i] == mim(l):
                        self.R('tempColumn<-c(tempColumn, "'+names[i]+'")', wantType = 'NoConversion')
                else:
                    if l[i] == max(l):
                        self.R('tempColumn<-c(tempColumn, "'+names[i]+'")', wantType = 'NoConversion')
