        ...
        try:
            ...
            if function in ['log10', 'log2', 'as.numeric', 'as.character', 'exp']:
                ...
                if function == 'toDateTime (MDY)':
                    self.R(self.data+'$dateAsMDY'+str(self.counter)+'<-strptime('+topText+', "%m/%d/%y")', wantType = 'NoConversion')
                elif function == 'toDateTime (YMD)':
                    self.R(self.data+'$dateAsMDY'+str(self.counter)+'<-strptime('+topText+', "%y/%m/%d")', wantType = 'NoConversion')
                elif function == 'toDateTime (DMY)':
                    self.R(self.data+'$dateAsMDY'+str(self.counter)+'<-strptime('+topText+', "%d/%m/%y")', wantType = 'NoConversion')
